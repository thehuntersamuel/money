...
For continuous SIP, run `node server/sip-worker.mjs` only in an approved persistent SERVER runtime with Node 24+, server-secret provisioning and a bounded symbol universe (`MORROW_SYMBOLS`, maximum 30). It exits disabled before reading provider values unless enabled; it requires a persistent `MORROW_STATE_DIR` and operator-controlled `MORROW_STREAM_RUN_ID`. Its five-attempt budget survives process restarts and stops on persistence failure. Set `MORROW_PARK_ON_FAILURE=true` on an automatically restarted host. See `ALPACA_STREAM_DEPLOYMENT.md` and the paused root `render.yaml` for the prepared deployment, cost approval, secure provisioning and runtime verification. It writes no credentials to output. Persist health receipts; stale heartbeats/coverage must remain visibly unknown.
