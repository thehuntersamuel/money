...
### Thesis review versions
Read the proposal's `thesis_version` from bridge state before revising it. A research/review edit must send exactly the next version and a strictly newer `last_researched_at`; lifecycle-only closing retains the current version. Read back the version from the write receipt/state. Old first-touch events remain immutable but do not attach to the new version. On a concurrent-update conflict, refetch and review the current state; never blindly bump/retry or manufacture an acknowledgment.
