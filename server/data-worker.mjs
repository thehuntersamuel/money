...
import {SOURCE_DATASETS,fetchSource} from './source-pipelines.mjs';
...
export async function ingestOnce(input,{config,db,fetchImpl=fetch,now=()=>new Date().toISOString()}){
 const {dataset,symbols=['SPY'],cik,start,end}=input;
 const began=now();universe(symbols);
 if(!['alpaca_sip','tiingo_eod','sec_submissions',...SOURCE_DATASETS].includes(dataset))throw Error('unsupported ingestion dataset');
 const health={dataset,checked_at:began,status:'blocked',detail:'not configured',coverage:'unknown'};
 try{
  let payload,provider,displayAllowed=false,provenance={};
  if(SOURCE_DATASETS.includes(dataset)){
   ...
   if(!config.tiingoKey||config.tiingoLicensed!==true||config.tiingoArchiveApproved!==true){health.detail='keys_license_or_archive_missing';return health;}
  ...
  const result=await db.from('morrow_data_snapshots').insert({provider,dataset,received_at:now(),display_allowed:displayAllowed,payload,provenance});if(result.error)throw Error('snapshot persistence failed');
