...
export function evaluateOpportunities(rows,{fixedCosts=null,periodStart,periodEnd}={}){
 ...
 const seen=new Set(),receipts=new Set(),eligible=[],missing=[];
 for(const r of rows){
  if(!r?.id||seen.has(r.id))throw Error('duplicate or missing opportunity ID');seen.add(r.id);
  if(r.cohort!=='paper')continue;
  if(r.receipt_id){if(receipts.has(r.receipt_id))throw Error('duplicate canonical close receipt');receipts.add(r.receipt_id);} // Shadow/rejected are reported separately, never pooled.
 ...
 return {verification:'unverified_reported_accounting',canonical_values_reconciled:false,units:'USD',period_start:periodStart,period_end:periodEnd,paper_opportunities:eligible.length,missing,shadow_count:rows.filter(r=>r.cohort==='shadow').length,rejected_count:rows.filter(r=>r.cohort==='rejected').length,
  ...
  uncertainty:'Unverified reported accounting: capital, P&L, timing and baseline inputs have not been reconciled to canonical records. Descriptive accounting; correlated opportunities are not independent statistical evidence',promotion_allowed:false};
}
