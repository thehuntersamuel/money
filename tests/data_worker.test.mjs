...
test('Tiingo persists normalized snapshot with raw display disabled by default',async()=>{
 const s=store();const r=await ingestOnce({dataset:'tiingo_eod',symbols:['SPY'],start:'2026-09-04',end:'2026-09-05'},{config:{tiingoKey:'TEST',tiingoLicensed:true},db:s.db,fetchImpl:async()=>new Response(JSON.stringify([{date:'2026-09-04T00:00:00Z',close:100,adjClose:95}]))});
...
test('SIP replay is honestly limited and persists durable observations',async()=>{
 ...
 const r=await ingestOnce({dataset:'alpaca_sip',symbols:['SPY']},{config:{alpacaKey:'TEST',alpacaSecret:'TEST',alpacaLicensed:true},db:s.db,fetchImpl,now});
