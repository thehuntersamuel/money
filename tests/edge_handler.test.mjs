...
function harness(){
 ...
 const db={from(table){tables.push(table);const q={then(resolve){return Promise.resolve({data:table==='v_paper_books'?[book]:[],error:null}).then(resolve)}};for(const method of ['select','eq','limit','order','in'])q[method]=()=>q;return q;},rpc(){mutations++;throw new Error('unexpected mutation')}};
